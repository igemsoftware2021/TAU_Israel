...
runtimes_for_tax = 10
translation_function = 'single_codon_local'
...
for tax_con, color in tax_to_color.items():
    print(tax_con, color)
    np_runs_for_tax = np.empty(shape=((len(list(range(2,len(org_dict),2)))), runtimes_for_tax))
    for i in range(runtimes_for_tax):
        tic = time.time()
        scores_for_x = []
        for x in range(2,len(org_dict),2):
            print('****', x)
            inner_tic = time.time()
    ...
    sub_microbiome_size = np.array(list(range(2,len(org_dict),2)))
